...
今天协助工厂解决X1C1的产测问题时，发现一个bug：扫码过站时失败，失败原因是设备二维码undefined。自己测试时没发现这个问题，原因是没测过站功能。设计测试用例的能力急需提高。

### 考虑发布环境

今天工厂那边反馈X1C1的自动扫码无法进行，经排查是访问`/dev/ttyACM0`权限不够，这个问题在调试时也遇到过，当时网上搜索方案解决了，但没有记录下来，也没考虑到生产时也会有同样的问题。太年轻了。

另外写的自动安装脚本也无法正常运行，原因是工厂电脑使用中文语言，脚本中拷贝文件到`~/Desktop`失败了。发布环境的差异性确实也是个需要考虑的问题。

## 2021年5月8日

### 养成设计测试用例的习惯

昨天修改X1Q2的镭雕二维码，只在python控制台简单测试了一下就发布了，今早工厂那边反馈软件无法启动，经调查才发现有bug：字符串中误用反斜杠导致json解析失败。

这次经历再次反映，自己在代码测试这方面做得确实很糟糕，有待改进。

### 提高程序调试手段

处理C1售后问题往往束手无策，原因是调试手段不够完善。有待深思。

### 考虑清楚程序异常级别

处理C1售后问题时，发现自己写的主机服务对一些致命错误没能报告出来，这是个很糟糕的表现。急需改进。

...
1. 物理页帧数量为4，虚拟页访问序列为 0,3,2,0,1,3,4,3,1,0,3,2,1,3,4 ，请问采用CLOCK置换算法（用1个bit表示存在时间）的缺页次数为（）
...
> 答案是9.我做了3次才做对，这种题需要画图一步步考察。

2. 物理页帧数量为4，虚拟页访问序列为 0,3,2,0,1,3,4,3,1,0,3,2,1,3,4 ，请问采用CLOCK置换算法（用2个bit表示存在时间）的缺页次数为（）
...
> 答案是7. 注意访问次数的取值共三个：00,01和10，即0~2. [答案解析](https://github.com/chyyuu/os_course_exercises/blob/master/all/04-2-quiz.md)

## 第十讲 实验三 虚拟内存管理

1. lab3中虚存管理需要直接借助的机制包括()
- [x] 页映射机制
- [ ] 段映射机制
- [x] 中断异常处理机制
- [x] IDE硬盘读写机制
...
1. 操作系统来维护一组队列，表示系统中所有进程的当前状态，有关管理进程的描述正确的是（）
- [x] 就绪态进程维护在进程就绪队列中
- [x] 等待态进程维护在进程等待队列中
- [ ] 运行态进程维护在进程运行队列中
- [ ] zombie态进程不在任何队列中
